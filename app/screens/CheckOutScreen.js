...
function CheckOutScreen({ navigation }) {
  ...
  const initializePayment = async () => {
    try {
      const response = await fetch(
        "https://desolate-tor-81663.herokuapp.com/payment",
        {
          method: "POST",
          headers: {
  ...
  return (
    <StripeProvider publishableKey="pk_test_51JOzfFSGs3WteDI290yrM0bhrCjRDXsZISCi8PVHG45isfw7CN09fsOooDB99yl042wgNGVE1G9p8a6sLo5MC1ZD00PovwK3x6">
      <View style={{ flex: 1 }}>
        <FlatList
              ...
              {add.map((d, index) => (
                <View key={index} style={styles.containerE}>
                  ...
                  <View style={styles.containerD}>
                    <View style={{ width: "100%" }}>
                      <AppText>
                        Address : {d.address}, {d.city}, {d.pinCode}, {d.state}
                      </AppText>
                    </View>
                  </View>
                </View>
